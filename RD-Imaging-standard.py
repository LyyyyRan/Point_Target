...
# Lyyy 2025.09.08
#
#       Pulse Compression over Azimuth have not been realized yet cuz by unknow reasons
#########################################################################################################
...
print(Xmin)
view_azimuth = int((Xmin + 50 * DX) * 512 / Lsar)
...
# 方位向下变频: 去除载波得到基带信号
DownConversion_Component = np.exp(-2j * np.pi * Fc * sn)
DownConversion_Component = np.dot(pointwise_apostrophe(DownConversion_Component), np.ones([1, M]))

# Point-Wise Multiply:
Data = Srnm * DownConversion_Component
...
Sr = FFT_Range(Srnm, shift=True)  # s(tr, ta) to S(fr, ta):
Img_AfterRangePulseCompression = Sr * H_r  # Pulse Compression over Range
Img_AfterRangePulseCompression = IFFT_Range(Img_AfterRangePulseCompression, shift=True)  # S(fr, ta) to s(tr, ta):
Img_AfterRangePulseCompression_Mod = np.abs(Img_AfterRangePulseCompression)  # Get Modulus

plt.figure('Modulus After Pulse Compression over Range')
plt.plot(Img_AfterRangePulseCompression_Mod[view_azimuth, :])
plt.xlabel('Range')
plt.ylabel('Modulus')
plt.xticks(ticks=[0, 128, 256, 384, 512, 640, 768, 896, 1024], labels=[f"{i}" for i in np.around(np.linspace(Yc - Y0, Yc + Y0, 9)).astype(int)])
...
plt.imshow(Img_AfterRangePulseCompression_Mod)  # 距离向压缩，未校正距离徙动的图像
plt.xticks(ticks=[0, 128, 256, 384, 512, 640, 768, 896, 1024], labels=[f"{i}" for i in np.around(np.linspace(Yc - Y0, Yc + Y0, 9)).astype(int)])
plt.yticks(ticks=[0, 64, 128, 192, 256, 320, 384, 448, 512], labels=[f"{i}" for i in np.around(np.linspace(Xmin - Lsar / 2, Xmax + Lsar / 2, 9)).astype(int)])
plt.xlabel('Range')
plt.ylabel('Azimuth')
...
# s(tr, ta) to S(tr, fa), that is to call Range-Doppler Domain
Sa_RD = FFT_Azimuth(Img_AfterRangePulseCompression, shift=True)
...
Img_AfterRCMC = Sa_RD
Img_AfterRCMC_Mod = np.abs(Img_AfterRCMC)

# S(tr, fa) to S(tr, ta)
Img_AfterRCMC = IFFT_Azimuth(Img_AfterRCMC, shift=True)

plt.figure('Modulus After RCMC')
plt.plot(Img_AfterRCMC_Mod[view_azimuth, :])
plt.xlabel('Range')
plt.ylabel('Modulus')
plt.xticks(ticks=[0, 128, 256, 384, 512, 640, 768, 896, 1024], labels=[f"{i}" for i in np.around(np.linspace(Yc - Y0, Yc + Y0, 9)).astype(int)])
...
plt.imshow(Img_AfterRCMC_Mod)  # 距离向压缩，未校正距离徙动的图像
plt.xticks(ticks=[0, 128, 256, 384, 512, 640, 768, 896, 1024], labels=[f"{i}" for i in np.around(np.linspace(Yc - Y0, Yc + Y0, 9)).astype(int)])
plt.yticks(ticks=[0, 64, 128, 192, 256, 320, 384, 448, 512], labels=[f"{i}" for i in np.around(np.linspace(Xmin - Lsar / 2, Xmax + Lsar / 2, 9)).astype(int)])
plt.xlabel('Range')
plt.ylabel('Azimuth')
...
# f for Azimuth direction (Doppler Frequency):
ta_gap = PRF / Nslow
fa_axis = np.fft.fftshift(np.arange(-Nslow / 2, Nslow / 2, 1)) * ta_gap  # 方位向频率轴
fa_axis = np.expand_dims(fa_axis, axis=1)  # (Na, ) to (Na, 1)

# Get H_a:
H_a = np.exp(-1j * np.pi / Ka * fa_axis ** 2)  # exp(-j * pi * ( (f_a**2) / (Ka) ))

# Pulse Compression over Azimuth:
Img_AfterAzimuthPulseCompression = Img_AfterRCMC * H_a

# S(tr, fa) to s(tr, ta):
Img_AfterAzimuthPulseCompression = IFFT_Azimuth(Img_AfterAzimuthPulseCompression, shift=True)

# Get Modulus:
Img_AfterAzimuthPulseCompression_Mod = np.abs(Img_AfterAzimuthPulseCompression)

plt.figure('Modulus Pulse Compress over Azimuth')
plt.plot(Img_AfterAzimuthPulseCompression_Mod[view_azimuth, :])
plt.xlabel('Range')
plt.ylabel('Modulus')
plt.xticks(ticks=[0, 128, 256, 384, 512, 640, 768, 896, 1024], labels=[f"{i}" for i in np.around(np.linspace(Yc - Y0, Yc + Y0, 9)).astype(int)])
...
fig, ax = plt.subplots()
plt.imshow(Img_AfterAzimuthPulseCompression_Mod)  # 距离向压缩，未校正距离徙动的图像
plt.xticks(ticks=[0, 128, 256, 384, 512, 640, 768, 896, 1024], labels=[f"{i}" for i in np.around(np.linspace(Yc - Y0, Yc + Y0, 9)).astype(int)])
plt.yticks(ticks=[0, 64, 128, 192, 256, 320, 384, 448, 512], labels=[f"{i}" for i in np.around(np.linspace(Xmin - Lsar / 2, Xmax + Lsar / 2, 9)).astype(int)])
ax.axis('scaled')
plt.xlabel('Range')
...
print('result.dtype', Img_AfterAzimuthPulseCompression.dtype)
